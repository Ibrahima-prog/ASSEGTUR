<?php

return [

    // NAVBAR

    'Home' => 'Anasayfa',
    'Home2' => 'Anasayfa',
    'OurAssociation' => 'Derneğimiz',
    'OurAssociation2' => 'Derneğimiz',
    'OurHistory' => 'Tarihçesi',
    'OurHistory2' => 'Tarihçesi',
    'AboutUs' => 'ASSEGTUR',
    'OurMissionAndVision' => 'Misyonumuz, Planımız ve Vizyonumuz',
    'Register' => 'Şimdi üye ol',
    'FAQs' => 'FAQs',
    'Gallery' => 'Galeri',
    'News' => 'Haber',
    'Activities' => 'Faaliyetler',
    'RecentActivities' => 'Son Faaliyetler',
    'All'=>'Tüm',
    'AcademicWorks' => 'Akademik Çalışmalar',
    'AcademicWorks2' => 'Akademik Çalışmalar',
    'Articles' =>'Makaleler',
    'Announces' =>'Duyurular',
    'Communities' => 'Topluluklar',
    'SuAKTAT' => 'SÜ AKTAT',
    'ContactUs' => 'İletişim',
    'activities_and_galleries'=>'Faaliyetler Ve Galeriler',
    'activities_and_galleries2'=>'Faaliyetler Ve Galeriler',

    //FOOTER
    'AssociationDesEtudiantsGuinéensDelaTurquie'=>"Türkiye'deki Gineli Öğrenciler Birliği",
    'AllRightsReserved' => 'Tüm hakları Saklıdır',
    'OurContacts' => 'İletişim Bilgileri',
    'Subscribe' => 'Abone ol',
    'Phone' => 'Telefon',
    'Email' => 'E-posta',
    'Newsletter' => 'Haber Bülteni',
    'SubscribMessage' => 'Bültenimize abone olun ve derneğimizle ilgili en son haberler ve güncellemelerden haberdar olun!',
    'usefullinks'=>'Kullanışlı Bağlantılar',

    // HOMEPAGE

    'Login' => 'Giriş',
    'Registration' => 'Şimdi üye ol',
    'Registration2' => 'Şimdi üye ol',
    'OurMission' => 'Misyonumuz',
    'OurPlan' => 'Planımız',
    'OurVision' => 'Vizyonumuz',
    'BecomeMember' => 'Afrokonya Üyesi Ol',
    'Registernow' => 'ŞİMDİ ÜYE OL',
    'LatestNews' => 'Son Haberler',
    'LatestNewsMessage' => ' Afrokonya\'nın son haberler </br> Konya\'daki Afrika Öğrencileri </br> Birliğin\'nin son haberleri, </br>tarkipte kalın',
    'Unity' => 'Birlik',
    'UnityText' => 'Afrokonia\'nın temel amacı Konya\'daki Afrikalı öğrencileri biraraya getirmektir.',
    'Solidarity' => 'Beraberlik',
    'SolidarityText' => 'Birliğimizin öğrencilerinin beraberlik içerinde yaşamaları için çalışıyor.',
    'Progress' => 'Gelişme',
    'ProgressText' => 'Derneğin ve Afrika\'nın gelişmesi hedefimizdir. Ciddi çalışmalar yürütmekteyiz',
    'UpcommingActivities' => 'Gelen Etkinlikler',
    'UpcommingActivitiesMessage' => 'Afrokonya\'nın gelen etkinliklerimiz, </br>Daha fazla bilgi için bizimle irtibata geçiniz',
    'PresentationVideo' => 'Tanıtım Video',
    'OurUniversities' => 'Kurumlar',
    'UniversityText' => 'Konya\'daki bulunan Üniversite ve okullarımız. Butun üyelerimiz bu kurumlarının öğrencisidir.',
    'language'=>'Dil',

    //REGISTER PAGE

    'becomeMember' => 'Üye Ol',
    'RegisterMessage' => 'Kayıt olun ve tam üye olun.',
    'YourPersonalDetails' => 'Kişisel Bilgileriniz',
    'Conditions' => 'Üyelik şartlar',
    'MoreDetails' => 'Detaylar',
    'Register' => 'Kaydol',
    'Cancel' => 'İptal',



    'fullname' => 'Ad Soyad',
    'fullnameText' => 'Tam adınız ve soyadınız giriniz',
    'selectGender' => 'Bir Cinsiyet Seçin',
    'Male' => 'Erkek',
    'Female' => 'Kadın',
    'selectcountry' => 'Bir ülke seçiniz',
    'homeAddress' => 'Ev adresiniz',
    'homeAddressText' => 'Ev adresinizi giriniz',
    'selectEducation' => 'Bir eğitim seviyesi seçiz',

    'Highschool' => 'Lise',
    'Bachelor' => 'Lisans',
    'Masters' => 'Yüksek Lisans',
    'php' => 'PhD',
    'UniversityInstitution' => 'Üniversite / Kurum Adı',
    'department' => 'Bölümü',
    'Success' => 'Bilgileriniz başarıyla kaydedildi',


    //FAQs PAGE

    'faqs' => 'Sıkça Sorulan Sorular',
    'GeneralQuestions' => 'Genel Sorular',
    'AssociationRelated' => 'Birlik Çalışmaları',
    'Membership' => 'Üyelik',
    'Others' => 'Diğer',

     //NEWS PAGE

    'RecentNews' => 'Son Haberler',

     //ACTIVITIES PAGE

    'All' => 'Tüm',
    'Upcomming' => 'Gelen Etkinlikler',
    'PastActivities' => 'Geçmiş Etkinlikler',
    'Summary' => 'Faaliyetin özeti',
    'Images' => 'Galeri',

    //ARTICLES PAGE
    'AcademinArticles' => 'AKADEMİK MAKALELER',
    'LatestArticles' => 'En son makaleler',
    'ReadMore' => 'Fazla Oku',
    'DownloadPDF' => 'PDF İndir',
    'CreatedAt' => 'Yayın tarihi',
    'Author' => 'Yazar',
    'Departments' => 'Bölümler',
    'Faculties' => 'Fakülteler',

     //AKTAT PAGE

    'AktatCommunity' => 'AKTAT TOPLULUĞU',
    'aktatYonetim' => 'Topluluk Yönetimi',

    //CONTACT PAGE

    'Email' => 'E-Posta',
    'SendMessage' => 'Bize mesaj bırakın.',
    'Submit' => 'GÖNDER',

    'yourname' => 'Ad ve Soyad',
    'email' => 'E-posta',
    'phonenumber' => 'Telefon',
    'subject' => 'Konu',
    'message' => 'Mesaj',
    'Address' => 'Adres',

    // ABOUT US

    'ExecutiveBoard' => 'Yönetim Kurulu',


];
